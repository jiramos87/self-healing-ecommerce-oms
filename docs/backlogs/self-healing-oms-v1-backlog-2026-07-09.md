...
| B11 | done | 2026-07-10 | 9789c0b | Self-contained demo dashboard served at GET / by the existing FastAPI app (app/ui.py serves app/static/index.html; wired ahead of the API routers in app/main.py). Single file, inline CSS/JS, no build step, no external hosts (CSP-friendly), data URI favicon. Pure client of the read API + rate-limited simulate: caps/health strip, five simulate buttons, live incident list with status badges + recurrence chips, expandable per-step trace timeline (served_by/latency/time) with real GitHub issue/PR links, recent-orders list. Auto-polls 3s while an incident is fresh (created <120s) and non-terminal, backs off to a 15s heartbeat when settled (guards against orphaned received rows polling hot forever); honors 429 (retry countdown + disabled buttons) and 503 (honest temporarily-unavailable, auto-recover); network-error resilient (recovered from a mid-restart ERR_CONNECTION_REFUSED). Verify gate green: ruff + pyright 0 + pytest 78 passed (tests/test_ui.py: GET / is HTML, five classes present, no admin/token surface, no external hosts, JSON endpoint paths unchanged). Deployed straight to main (git-connected auto-redeploy, sfo1). Live proof on the deployed URL: dashboard renders from live API; clicking unknown_region drove a new incident received -> issue_opened -> pr_opened in ~24s (issue #20 + PR #21), trace 7 steps, both GitHub links live, caps ticked 7->8/20, poller returned to idle. Left the .gitignore .vercel change out of the commit. |
| B12 | done | 2026-07-10 | e062f52 | Scheduled GitHub Action .github/workflows/pr-hygiene.yml (daily cron 07:00 UTC + workflow_dispatch with max_age_hours/dry_run inputs). One actions/github-script@v7 step, default GITHUB_TOKEN (contents/issues/pull-requests: write), concurrency-guarded, idempotent. Logic: (1) close open PRs on agent/fix-* branches older than the cutoff and delete their head branch; (2) close open issues titled "[agent] ..." older than the cutoff (state_reason not_planned) since closing a PR does not close its "Fixes #n" issue; (3) prune orphan agent/fix-* branches that have no open PR and whose last commit predates the cutoff (catches merged-PR and crashed-run leftovers, commit-age guard avoids racing a fresh run). Never touches human PRs/issues or main (head-ref prefix + title-prefix filters). Fixed a parse bug so max_age_hours=0 is honored (Number(x)||24 coerced 0 to 24). Acceptance proven: workflow registered/active under Actions (id 310894973); dry-run dispatch (max_age_hours=0, dry_run=true) reported closedPRs=[21], closedIssues=[20], deletedBranches=[agent/fix-0c0efc6ffcf2 (PR #21 head), agent/fix-d15e404c5e90 (orphan from merged PR #19)], keptPRs=[]; verified non-destructive afterward (PR #21 OPEN, issue #20 OPEN, branch still present). Proof artifacts #20/#21 left open for B13 screenshots; the daily schedule cleans them at the 24h threshold. |
| B13 | pending | - | - | - |
...
- Note (2026-07-09, B10 setup): Vercel project `self-healing-ecommerce-oms` created under `jiramos87s-projects` and connected to the GitHub repo, so merged fix PRs auto-redeploy and the self-healing loop closes without manual steps. Function pinned to `sfo1` in vercel.json because the Railway database proxy resolves to San Francisco while Vercel defaults to `iad1`; that cross-country hop is what put GET p95 over the PRD's 500ms bar during B09, so the p95 bar must be re-measured against the deployed URL, not locally. Branch protection on `main`: pull request required, force pushes and deletions blocked, `enforce_admins=false` so the owner can still push directly. Honest limit: GITHUB_FIX_PAT acts as the repo owner, so it retains admin bypass and the "agent never merges" invariant is still enforced by the code having no merge call, not by the platform. Real token-level enforcement needs a separate machine account or a GitHub App with PR-create-only rights (v1.1 hardening; good README material).
