...
| B10 | blocked | 2026-07-09 | 23b04bf | Artifacts done + build gate green; live acceptance BLOCKED on needs-human. Built: vercel.json (single-function FastAPI, maxDuration 60, excludeFiles {tests,scripts}); scripts/launch_gate.py (20 deliveries across all 5 classes + recurrence + merge-closes-loop, pass/fail table, non-zero exit on fail); docs/deploy.md. Out-of-Files touch (necessary): pyproject.toml [tool.vercel] entrypoint="api.index:app" to disambiguate the two entrypoint candidates (api/index.py, app/main.py) and force whole-app single-function routing. Design note: gate delivers signed webhooks directly (public /demo/simulate is capped 3/10min per IP, which 20 rapid runs trip); reuses app.generators for novelty. Build gate: ruff clean + pyright 0 + pytest 70 passed + uvicorn boot /health 200. NEEDS-HUMAN to close: create/link Vercel project, provision runtime env (real secrets not available to agent), deploy prod, run `python scripts/launch_gate.py --base-url <url>` with WEBHOOK_SECRET+DATABASE_URL, merge the E1 PR mid-gate; then record gate output here and flip to done. |
| B11 | pending | - | - | - |
